...
class BulkWriter
{
     ...
     * [https://cloud.google.com/firestore/docs/reference/rpc/google.firestore.v1beta1#batchwriterequest](BatchWriteRequest) when sufficient
     * number of operations are enqueued. */
     ...
     * legacy WriteBatch.
     */
    public function __construct(ConnectionInterface $connection, $valueMapper, $database, $options = [])
    {
    ...
    public function flush(bool $waitForRetryableFailures = false)
    {
        while (true) {
            ...
            for ($i = 0; $i < $batchSize; $i++) {
                $writeResult = $response['write_results'][$i];
                $status = $response['status'][$i];
                $this->finalResponse['write_results'][$batchIds[$i]] = $writeResult;
                $this->finalResponse['status'][$batchIds[$i]] = $status;
                if ($status['code'] !== Code::OK) {
                    $this->handleSendbatchFailure($writesBatch[$i], $status['code']);
                }
            }
        }
    }
     ...
     * @param Code $lastRunStatusCode
     * @return void
     */
    private function handleSendbatchFailure(int $writesId, Code $lastRunStatusCode)
    {
        if ($lastRunStatusCode === Code::OK) {
            return;
        }
        // TODO: add a check whether this error is retryable else just ignore

        $numFailedAttempts = 1;
        $backoffDurationInMillis = 0;
        if (array_key_exists($writesId, $this->retryScheduledWrites)) {
            //TODO: check if MAX_RETRY_ATTEMPTS failures have reached
            // can be handled while querying the batch for failures
        ...
        $this->retryScheduledWrite[$writesId] = [
    ...
    {
        // todo: check scenario when some writes exceed max retries, cant keep trying forever
        $writesBatchIds = [];
        ...
        foreach (array_keys($this->writes) as $writeId) {
            if (count($writesBatchIds) >= $this->maxBatchSize) {
                break;
            }
            // ignore if write has a result and result is already successful
            if (array_key_exists($writeId, $this->finalResponse) &&
                $this->finalResponse[$writeId]['status']['code'] === Code::OK) {
                continue;
            }
    ...
    public function close()
    {
        $this->flush(true);
        $this->closed = true;
        // TODO: check whether a ksort is needed
        ksort($this->finalResponse['write_results']);
        ksort($this->finalResponse['status']);
        return $this->finalResponse;
    }

    // TODO: think of making handle retries / getBackoffDuration a public function instead
    public function getBackoffDuration(Code $lastStatus, int $backoffDurationInMillis = 0): int
    {
        if ($lastStatus === Code::RESOURCE_EXHAUSTED) {
            $backoffDurationInMillis = self::DEFAULT_BACKOFF_MAX_DELAY_MS;
        } else if ($backoffDurationInMillis <= 0) {
            $backoffDurationInMillis = self::DEFAULT_BACKOFF_INITIAL_DELAY_MS;
        } else {
            $backoffDurationInMillis *= self::DEFAULT_BACKOFF_FACTOR;
        }

        return $backoffDurationInMillis;
    }
    ...
    public function sendBatch(array $writes, array $options = [])
    {
        ...
        // TODO: implement exponential backoff here using BulkWriterOperation
        // basicaly transfer all logic of exception handling / retry from sendBatch to BulkWriterOperation
